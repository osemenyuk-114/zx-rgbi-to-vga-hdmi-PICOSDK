...
void start_dvi()
{
...
#ifdef DVI_USE_HSTX
  // hstx_div=2 for 640×480@60 and 720×576@50; hstx_div=1 for 800×600@75
  ...
  uint32_t hstx_div = video_mode.sys_freq == video_modes[MODE_800x600_75Hz]->sys_freq ? 1 : 2;
  ...
  sm_config_set_out_shift(&c_conv, true, false, 32); // shift right, no autopull
  ...
  channel_config_set_dreq(&c0, DREQ_PIO_DVI + SM_DVI_CONV);
  channel_config_set_chain_to(&c0, dma_ch1);

  dma_channel_configure(
      dma_ch0,
      &c0,
      &PIO_DVI->txf[SM_DVI_CONV],
      &v_out_dma_buf[0][0],
      whole_line / 4,
      false //
  );
  ...
  channel_config_set_dreq(&c2, DREQ_PIO_DVI + SM_DVI);
  ...
  dma_channel_configure(
      ...
      &PIO_DVI->txf[SM_DVI],
      palette,
      4,    // 4 × uint32_t per palette entry
      false //
  );
  ...
  channel_config_set_dreq(&c3, DREQ_PIO0_RX0 + SM_DVI_CONV);
  channel_config_set_chain_to(&c3, dma_ch2);

  dma_channel_configure(
      dma_ch3,
      &c3,
      &dma_hw->ch[dma_ch2].read_addr,
      &PIO_DVI->rxf[SM_DVI_CONV],
      1,
